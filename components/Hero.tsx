    ...
    <section id="hero" className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      {theme !== 'photo' ? (
        ...
        <div className="relative z-10 w-full h-full flex flex-col justify-end pb-24 items-center text-center px-4">
          ...
          <h1 className="text-3xl xs:text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight text-white mb-3 drop-shadow-2xl animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          ...
          <p className="mt-2 max-w-xl text-base xs:text-lg text-white/80 mx-auto mb-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            Trygghet i vann, mestring og glede.
          </p>

          <div className="flex flex-row gap-3 justify-center items-center animate-fade-in-up w-full px-2 mb-8" style={{ animationDelay: '0.3s' }}>
            <button
              onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
              className="group relative px-5 py-3 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-200 text-sm font-bold rounded-full transition-all border border-cyan-200/30 hover:border-cyan-200/50 backdrop-blur-sm overflow-hidden whitespace-nowrap"
            >
              <span className="relative z-10 flex items-center justify-center gap-2">
                Meld på kurs <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </span>
            </button>
            <button
              onClick={() => document.getElementById('schedule')?.scrollIntoView({ behavior: 'smooth' })}
              className="group relative px-5 py-3 bg-transparent overflow-hidden rounded-full transition-all hover:scale-105 border border-white/30 hover:border-white/60"
            >
              <div className="absolute inset-0 bg-white/10 group-hover:bg-white/20 transition-colors"></div>
              <span className="relative font-bold text-white text-sm tracking-wider flex items-center justify-center gap-2">
                SE KURSTIDER <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </span>
            </button>
          </div>
        </div>
